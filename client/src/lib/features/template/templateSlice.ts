...
// Create new template
export const createTemplate = createAsyncThunk("templates/create", async (templateData: Omit<Template, "id" | "createdAt" | "updatedAt">) => {
    const response = await axios.post(API_BASE_URL, templateData);
    return response.data as Template;
});

// Update template
export const updateTemplate = createAsyncThunk("templates/update", async ({ id, data }: {
    id: number;
...
// Delete template
export const deleteTemplate = createAsyncThunk("templates/delete", async (id: number) => {
...
const templateSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        .addCase(updateTemplate.fulfilled, (state, action: PayloadAction<Template>) => {
            const index = state.templates.findIndex((p) => p.id === action.payload.id);
        ...
        .addCase(deleteTemplate.fulfilled, (state, action: PayloadAction<number>) => {
            state.templates = state.templates.filter((p) => p.id !== action.payload);
        });
    },
});
